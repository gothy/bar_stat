...
(function() {
  var FeedParser, app, async, bcrypt, combine_mt_feed, express, fs, launch, moment, utils, uuid, __;
  ...
  app.post('/bar_uploads/file/:token', function(req, res, next) {
    ...
    if (file) {
      return db.hgetall("up." + token, function(err, reply) {
        if (reply && (reply.instid === instid) && (reply.partner === partner)) {
          db.hmset("up." + token, {
            name: file.name,
            path: file.path,
            type: file.type
